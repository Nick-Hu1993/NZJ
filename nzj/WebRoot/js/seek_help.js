...
function addHelp() {
		if($("#name").val() == '' || $("#phone").val() == '' || $("#content").val() == '') {
			...
			var formData = new FormData(document.getElementById("addHelp_form"));
			$.ajax({
				type: "post",
				url: mainUrl + "addHelp",
				data: formData,
...
var getJsonArrayByPageSize = function(pageSize, pageNo) {
	...
	if(indexState==0){
		$.ajax({
		type: "post",
		url: mainUrl + "getHelpListByStatus",
		dataType: "json",
		async: false,
		cache: false,
		data: {
...
var builderUQTQueryMsg = function(UQTQueryMsg) {
	...
	var th = '<tr><th scope="col" class="eng_name" >公司名称</th><th class="chi_name" scope="col">姓名</th><th scope="col"  class="chi_name">电话</th><th class="match_type" scope="col">求助内容</th><th scope="col"  class="dis_dta">操作</th><th class="dis_hidden" style="display: none">隐藏属性</th></tr>';
...
//		};
		...
		var company = eachData.company;
		var name = eachData.name;
		var phone = eachData.phone;
        var content = eachData.content;
        var u = '';
            u +="<td class='eng_name'>" + company + "</td>" 
			+"<td class='chi_name'>" + name + "</td>" 
			+"<td class='chi_name'>" + phone + "</td>" 
			+"<td class='match_type'>" + content + "</td>" 
			+"<td class='dis_dta'>" 
			if(indexState==0){
